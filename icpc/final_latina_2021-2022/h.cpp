...
class Solution {
    public:
        ...
        int cost[inf][inf]; //cost matrix
    int n,
    max_match; //n workers and n jobs
    int lx[inf],
    ly[inf]; //labels of X and Y parts
    int xy[inf]; //xy[x] - vertex that is matched with x,
    int yx[inf]; //yx[y] - vertex that is matched with y
    bool S[inf],
    T[inf]; //sets S and T in algorithm
    int slack[inf]; //as in the algorithm description
    int slackx[inf]; //slackx[y] such a vertex, that
    int prev_ious[inf]; //array for memorizing alternating p
    ...
    void augment() //main function of the algorithm
    {
        ...
        int q[inf], wr = 0, rd = 0; //q - queue for bfs, wr,rd - write and read
        //pos in queue
...
int main() {
    ...
    int tabela[n/2][n-n/2];
    int pares = n/2;
    int impares = n-n/2;
    for(int i = 0; i<pares; i++){
        for(int j = 0; j<impares; j++){
            tabela[i][j] = dist[(i+1)*2][(j*2)+1];
        }
        
    }
   
    int Arr[pares*impares];
    for(int i = 0; i<pares; i++){
        for(int j = 0; j<impares; j++){
            Arr[i*impares + j] = tabela[i][j];
        }        
    }

    int aux = max(pares, impares);
    Solution ob;
    cout << ob.assignmentProblem(Arr, aux)*2 << endl;
}